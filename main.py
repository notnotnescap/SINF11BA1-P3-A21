...
E=print
C=str
from microbit import button_a as H,button_b as I,pin_logo as W,sleep as J,display as A,Image as D,accelerometer as c,microphone as o
import music as d,sys,radio as F,random as p
q=O
...
def t():
	...
	if Q==1:
		I=p.randint(0,999999999);E(O.format(I));K(B,L,I);N=P(P(C(I)));E(S.format(N));A.show(D(e))
		while G:
			H=M(F.receive(),B)
			if H:
				if H[0]==L:
...
def f(message,key,decryption=O):
...
def K(key,t,content):B=content;A='{}|{}|{}'.format(t,C(X(C(B))),C(B));A=f(A,key);F.send(A)
class g:
	def __init__(A,id):A.id=id
class u(g):
	def __init__(A,id):super().__init__(id);A.quantite_de_lait=0;A.image_lait=j;A.index_menu=0;A.menu_items=[('C',A.mode_compteur),('S',A.mode_statut),('T',A.mode_temperature),('F',A.mode_find)];A.IMAGE_SLEEP_SEQUENCE=[D('00000:00000:99099:00000:09990'),D('00000:99990:00900:09000:99990'),D('09999:00090:00900:09999:00000')];A.menu()
	def menu(B):
		A.clear();J(100)
		while G:
	...
	def remove_lait(B):
		...
		B.image_lait=a.join(Y(i(C)));E(B.image_lait);E(B.quantite_de_lait);A.show(D(B.image_lait))
	def mode_compteur(B):
		...
		while not W.is_touched():
			if H.is_pressed()and I.is_pressed():
				...
				if H.is_pressed()and I.is_pressed():B.image_lait=j;B.quantite_de_lait=0;A.show(D(B.image_lait))
			elif H.is_pressed():
	...
	def main(D):
		...
		while G:
			for S in range(10000):
				N=M(F.receive(),B)
				if S%500==0:
					R=(c.get_x()+c.get_y()+c.get_z()+o.sound_level()*100)/4;T=abs((R-Q)/100);D.history.append(T);D.history.pop(0);P=sum(D.history)/X(D.history)
			...
			if D.statut!=D.old_statut:K(B,k,C(D.statut));D.old_statut=D.statut
			if N:
				if N[0]==l and N[2]==m:J(100);K(B,n,C(temperature()));E('TEMP: {}'.format(temperature()))
if not q:Q=r();A.show(C(Q));J(500);A.clear();t()
else:Q=0
h=s()
if h==S:w=u(Q)
elif h==T:w=v(Q)
