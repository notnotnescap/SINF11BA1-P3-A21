...
D=str
C=print
from microbit import button_a as G,button_b as H,pin_logo as U,sleep as J,display as A,Image as E,accelerometer as c,microphone as l
...
def p():
	D='ROLE';A.show('?')
	while F:
		if G.is_pressed():L(B,D,X);A.show(Y);J(1000);A.clear();C('Parent');return Y
		if H.is_pressed():L(B,D,Y);A.show(X);J(1000);A.clear();C('Child');return X
def N(string):
	B=string
	def C(value):
		A=value;A=A%2**32
...
def q():
	...
	if O==1:
		G=m.randint(0,999999999);C(P.format(G));L(B,M,G);K=N(N(D(G)));C(S.format(K));A.show(E(e))
		while F:
			H=Q(I.receive(),B)
			if H:
				if H[0]==M:
	...
	if O==2:
		...
		while F:
			...
			if H:
				if H[0]==M:break
		G=R(H[2]);C(P.format(G));K=N(N(D(G)));C(S.format(K));L(B,M,K);B=B+N(D(G));A.show(E.YES);J(500)
def K(button):
	A=button
	if A.lower()==Z:
...
def f(message,key,decryption=P):
	G=decryption;E='';H=V(key);F=[M(A)for A in key]
...
def L(key,t,content):B=content;A='{}|{}|{}'.format(t,D(V(D(B))),D(B));A=f(A,key);I.send(A)
class g:
...
class r(g):
	...
	def add_lait(B):
		...
		for D in W(J):
			if b in D and not H:
				I=D.rfind(b);G.append(D[:I]+T+D[I+1:])
				if B.quantite_de_lait>0:B.quantite_de_lait-=1
				H=F
			else:G.append(D)
		B.image_lait=a.join(W(G));C(B.image_lait);C(B.quantite_de_lait);A.show(E(B.image_lait))
	def remove_lait(B):
		I=B.image_lait.split(a);D=[];H=P
		for G in W(i(I)):
			if T in G and not H:
				D.append(G.replace(T,b,1))
				if B.quantite_de_lait<25:B.quantite_de_lait+=1
				H=F
			else:D.append(G)
		B.image_lait=a.join(W(i(D)));C(B.image_lait);C(B.quantite_de_lait);A.show(E(B.image_lait))
	def mode_compteur(B):
		A.show(E(B.image_lait));K('ab')
		while not U.is_touched():
	...
	def mode_temperature(B):
		while not U.is_touched():A.show('x')
		B.menu()
	def mode_find(D):
		while not U.is_touched():
			B=I.receive_full()
	...
	def main(E):
		...
		while F:
			for O in range(10000):
				U=Q(I.receive(),B)
				if O%500==0:
					N=(c.get_x()+c.get_y()+c.get_z()+l.sound_level()*100)/4;R=abs((N-M)/100);E.history.append(R);E.history.pop(0);C('\x1bc');J=sum(E.history)/V(E.history);C(J)
					if J<.8:E.statut=0;A.show(T)
					elif J<3.:E.statut=1;A.show('1')
					elif J<4.:E.statut=2;A.show('2')
					M=N
				if H.is_pressed()and E.statut>0:d.play(d.PYTHON,wait=P);K(S)
